...
#                                 
#                                 oxyzen_sensor_test()
#                                 rxData_oxyzen = uart0.read()
#                                 time.sleep(0.1)
#                                 
#                                 if not rxData_oxyzen == None:
#                                     uart1.write("OXYZEN_TEST_SUCCESS")
#                                     gui_led.toggle()
#                                     time.sleep(0.01)
...
while startflag:
    ...
    f = open("./ledchamber_loggingdata/ledchamber_loggingdata"+str(potime1[0])+str(potime1[1])+str(potime1[2])+str(potime1[4])+str(potime1[5])+str(potime1[6]),"w")
    ...
    picosleep.seconds(picosleeptime-600) #10분전에 깨어난다 EX) 대기시간이 120분이면 총 picosleep은 110분
    time.sleep(1)
    
    
    # 처음 모터를 돌려서 튜브에 물 에어 빼기
    first_motor_count = 1
    while True:
        if (first_motor_count == 1):
            stir_motor_start(stir_pump_rate)
        elif (first_motor_count == 180):
            stir_motor_start(0)
        elif (first_motor_count == 598): #10분에 가까워지면 while문 탈출해서 main문으로 들어간다
            break
...
#                     print("===============\n")
                    ...
                    f = open("./ledchamber_loggingdata/ledchamber_loggingdata"+str(potime1[0])+str(potime1[1])+str(potime1[2])+str(potime1[4])+str(potime1[5])+str(potime1[6]),"a")
                    ...
                    f.write(str(No_count+1)+" "+str(j)+" "+str(No_cycle+1)+" "+str(retime1[0])+"-"+str(retime1[1])+"-"+str(retime1[2])+" "+
                            str(retime1[4])+":"+str(retime1[5])+":"+str(retime1[6])+" "+str(float(int(ps_list_O[0])/100))+" "+str(float(int(ps_list_T[0])/100))+"\n")   
                    f.close()
                    No_count += 1
                    No_cycle += 1
                    ps_list_T.clear()
...
#                 print("===============\n")
                ...
                f.write(str(No_count+1)+" X "+str(No_cycle)+" "+str(retime1[0])+"-"+str(retime1[1])+"-"+str(retime1[2])+" "+
                        str(retime1[4])+":"+str(retime1[5])+":"+str(retime1[6])+" "+str(float(int(ps_list_O[0])/100))+" "+str(float(int(ps_list_T[0])/100))+"\n")   
                f.close()
                No_count += 1
                No_cycle += 1
